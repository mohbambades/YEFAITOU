...
Deno.serve(async (req) => {
  ...
  if (!PAYSTACK_SECRET_KEY || !SERVICE_ROLE_KEY) return new Response("Webhook not configured", { status: 503 });
  ...
  // Paystack uses the XOF amount directly (no *100 conversion for XOF).
  if (Number(payment.amount) !== Number(intent.amount_xof) || payment.currency !== intent.currency) {
    await admin.from("payment_intents").update({
      status: "failed",
      metadata: {
        ...intent.metadata,
        rejection: "amount_or_currency_mismatch",
